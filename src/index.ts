...
export class DiscordPlugin extends Plugin {
  ...
  async loadAddon(addons: DiscordAddon): Promise<void> {
    ...
    addons
      .execute(this.client)
      .then(() => {
        this.info("Addon", addons.name, "started!");
      })
      .catch((err) => {
  ...
  unloadAddon(addon: DiscordAddon): void {
    ...
    if (this.addons.has(addon.name)) {
      ...
      this.info("Addon " + addon.name + " unloaded!");
    }
  }
  ...
  async init(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      for (let guild of this.settings.guildIds) {
        ...
        rest
          .put(Routes.applicationGuildCommands(this.settings.clientId, guild), {
            body: [],
          })
          .then(() => this.info("Successfully cleaned up guild commands."))
          .catch(console.error);
        // push current ones
        rest
          .put(Routes.applicationGuildCommands(this.settings.clientId, guild), {
          ...
          .then(() => {
            this.info("Successfully registered application commands!");
            return resolve();
          })
          .catch((err) => {
  ...
  async execute(): Promise<void> {
    return new Promise(async (resolve, reject) => {
      ...
      client.on("interactionCreate", (interaction) =>
        this.interactionHandler(client, interaction, this.events)
      );
