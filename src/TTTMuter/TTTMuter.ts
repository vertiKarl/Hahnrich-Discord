...
export default class TTTMuter extends DiscordAddon {
    ...
    routes() {
        ...
        this.app.use("/", (req, res, next) => {
            ...
            this.log(`${req.ip} tried to request but was not authorized! (${req.headers.authorization})`);
        ...
        this.app.get("/id", (req, res, next) => {
            ...
            const found = this.guild!.members.cache.find(
                member =>
                    ...
                    member.displayName.toLowerCase().match(new RegExp(`.*${name.toLowerCase()}.*`)) ||
                    member.displayName.toLowerCase().match(new RegExp(`.*${nick.toLowerCase()}.*`))
            ...
            if (!found) {
                ...
                res.status(200).json({ name: found.displayName, nick: found.nickname, id: found.id }).end();
                this.log(`Success matched ${found.displayName} (${found.id}) to ${nick} (${name})`);
            }
        })
        ...
        this.app.post("/mute", async (req, res, next) => {
            ...
            if(id && status) {
                try {
                    ...
                    await member.voice.setMute(status, status ? "dead players can't talk!" : undefined);
                } catch(err) {
                    res.status(500).end();
            ...
            this.app.get("/", async (req, res, next) => {
                ...
                switch(req.headers.req) {
                    case "connect": {
                        ...
                        const found = this.guild!.members.cache.find(
                            member => member.displayName === tag || member.displayName.toLowerCase().match(new RegExp(`.*${tag.toLowerCase()}.*`))
                        );
        
                        if (!found) {
                            res.status(404).json({ answer: 0 });
                            this.error('[LegacyConnect][Error]', `0 users found with tag "${tag}".`);
                            return;
                        } else {
                            res.status(200).json({ tag: found.displayName, id: found.id });
                            this.log('[LegacyConnect][Success]', `Connecting ${found.displayName} (${found.id})`);
                            return;
                        }
                    }
                    case "sync": {
                    ...
                    case "mute": {
                        ...
                        if (typeof id !== 'string' || typeof mute !== 'boolean') {
                            res.status(400).json({ success: false, errorId: 'INVALID_PARAMS', errorMsg: 'ID or Mute value missing' }).end();
                            this.error('[LegacyMute][Missing Params]', `id: "${id}" (${typeof id}), mute: "${mute}" (${typeof mute})`);
                            return;
                        }
                        ...
                        try {
                            ...
                            res.status(200).json({ success: true });
                            this.log(`[LegacyMute][Discord:SetMute][Success]`, `${mute ? 'Muted' : 'Unmuted'} ${id}`);
                        } catch (err) {
                            res.status(500).json({ success: false, errorId: 'DISCORD_ERROR', errorMsg: err }).end();;
                            this.error(`[LegacyMute][Discord:SetMute][Error]`, `${mute ? 'Mute' : 'Unmute'}: ${id} - ${err}`);
                        }
                        break;
                    }
                }
